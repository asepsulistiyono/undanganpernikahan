...
import {
  Heart, MapPin, Calendar, Clock, Gift, Music, ChevronDown, MessageCircle,
  Send, Sparkles, ArrowRight, XCircle
} from 'lucide-react';

// ============================================================
// DEFAULT TEMPLATE DATA (dipindah ke atas, di luar komponen)
// ============================================================
const defaultTemplateData: WeddingData = {
  ...
  galleryImages: [],
  story:
    'Pertemuan kami dimulai dari sebuah kebetulan yang indah. Dari saling mengenal, kami menemukan bahwa kami saling melengkapi dalam setiap aspek kehidupan. Cinta kami tumbuh seiring waktu, dan kini kami siap melangkah ke jenjang yang lebih serius.',
  quote:
...
export default function Invitation() {
  ...
  const ownerDisplayName = ownerUsername
    ? store.getUserDisplayName(ownerUsername)
    : 'Wedding Template';

  const theme = themes.find(t => t.id === selectedTheme) || themes[0];
  const bodyFont =
    availableFonts.find(f => f.id === weddingData.customFont) || availableFonts[0];

  // ---------- COPY TO CLIPBOARD (sekarang weddingData sudah ada) ----------
  const copyToClipboard = async (text: string, accountId: string) => {
    const bankName =
      accountId === 'bank1' ? weddingData.bankName : weddingData.bankName2;
    try {
      ...
      const loadData = async () => {
        try {
          ...
          if (!foundUser) {
            ...
            const wd = await firebaseService.getWeddingData(user);
            console.log('Loaded wedding data from Firebase:', wd);

            if (wd) setFirebaseWeddingData(wd);

            const themeFromFb = await firebaseService.getTheme(user);
            setFirebaseTheme(themeFromFb || 'elegant-gold');

            const liveFromFb = await firebaseService.getLiveStatus(user);
            setFirebaseIsLive(liveFromFb);
          }
        } catch (error) {
  ...
  useEffect(() => {
    ...
    const targetDate = new Date(
      `${weddingData.weddingDate}T${weddingData.weddingTime}:00`
    ).getTime();
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-950 text-white">
        <div className="text-center px-6">
          ...
          <p
            ...
            style={{ fontFamily: bodyFont.family }}
          >
            Undangan ini belum dipublikasikan oleh pemilik.
          </p>
          <a
            href="#/admin"
            className="inline-block px-6 py-3 bg-amber-500 text-white rounded-lg hover:bg-amber-600 transition"
    ...
    return (
      ...
      >
        <div className="absolute inset-0 overflow-hidden">
          <div
            className="absolute top-1/4 left-1/4 w-64 sm:w-96 h-64 sm:h-96 rounded-full animate-pulse"
            style={{
          ...
          />
          <div
            className="absolute bottom-1/4 right-1/4 w-56 sm:w-80 h-56 sm:h-80 rounded-full animate-pulse"
            style={{
              background: `radial-gradient(circle, ${theme.accentColor}25, transparent)`,
              filter: 'blur(80px)',
        ...
        <div
          className="absolute w-64 h-64 rounded-full pointer-events-none transition-all duration-300 ease-out hidden md:block"
          style={{
        ...
        <div className="text-center px-4 sm:px-6 relative z-10 w-full max-w-md">
          ...
          <p
            className="text-xs uppercase tracking-[0.3em] sm:tracking-[0.4em] mb-4 sm:mb-6 opacity-60"
            style={{ color: theme.textColor }}
          >
            ✦ The Wedding Of ✦
          </p>
          <h1
            className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-bold animate-fade-in-up break-words"
            style={{
          ...
          {guestName && (
            <div className="mb-6 sm:mb-8 inline-block w-full">
              <div
                className="px-4 sm:px-6 py-3 sm:py-4 rounded-2xl backdrop-blur-md border"
                style={{
              ...
              >
                <p
                  className="text-xs uppercase tracking-wider opacity-50 mb-1"
                  style={{ color: theme.textColor }}
                >
                  Kepada Yth.
                </p>
                <p
                  className="text-xs sm:text-sm opacity-70 mb-1"
                  style={{ color: theme.textColor }}
                >
                  Bapak/Ibu/Saudara
                </p>
          ...
          <div className="relative inline-block mt-4 sm:mt-0">
            <button
              onClick={handleOpen}
              className="relative px-8 sm:px-10 py-3.5 sm:py-4 rounded-full font-medium text-sm transition-all hover:scale-105 group overflow-hidden min-w-[200px]"
              style={{
          ...
          <ChevronDown
            className="w-5 h-5 mx-auto mt-8 sm:mt-10 animate-bounce opacity-40"
            style={{ color: theme.textColor }}
          />
        </div>
      </div>
  ...
  return (
    <div
      style={{ background: theme.bgGradient, minHeight: '100vh', fontFamily: bodyFont.family }}
      className="relative"
    >
      <button
        onClick={() => setIsPlaying(!isPlaying)}
        className="fixed top-4 right-4 z-50 w-10 h-10 sm:w-11 sm:h-11 rounded-full flex items-center justify-center shadow-lg backdrop-blur-md border transition-all hover:scale-110"
        style={{
          background: theme.cardBg,
          borderColor: `${theme.primaryColor}30`,
          color: theme.primaryColor,
      ...
      <section className="min-h-screen flex items-center justify-center relative overflow-hidden px-4 sm:px-6">
        <div className="absolute inset-0">
          <div
            ...
            style={{
              background: `radial-gradient(circle, ${theme.primaryColor}15, transparent)`,
              filter: 'blur(80px)',
              animationDuration: '5s',
            }}
          />
          <div
            className="absolute bottom-1/3 right-1/4 w-80 h-80 rounded-full animate-pulse"
            style={{
              background: `radial-gradient(circle, ${theme.accentColor}10, transparent)`,
              filter: 'blur(60px)',
        ...
        {weddingData.coverImage && (
          <div className="absolute inset-0">
            ...
            <div
              ...
              style={{ background: theme.bgGradient, opacity: 0.85 }}
            />
          </div>
        )}
        ...
        <div className="text-center relative z-10 py-20">
          <div className="flex items-center justify-center gap-3 mb-8">
            <div
              className="w-24 h-px"
              style={{
                background: `linear-gradient(to right, transparent, ${theme.primaryColor}60)`,
              }}
            />
            ...
            <div
              className="w-24 h-px"
              style={{
                background: `linear-gradient(to left, transparent, ${theme.primaryColor}60)`,
              }}
            />
          </div>
          <p
            className="text-xs uppercase tracking-[0.4em] mb-8 opacity-60"
            style={{ color: theme.textColor }}
          >
            ✦ The Wedding Of ✦
          </p>
          ...
          <div className="flex items-center justify-center gap-3 sm:gap-6 mb-6 sm:mb-8">
            {weddingData.groomPhoto ? (
              <div className="relative">
                <div
                  className="w-24 h-24 sm:w-28 sm:h-28 md:w-36 md:h-36 rounded-full overflow-hidden border-2 shadow-xl"
                  style={{
              ...
              <div
                className="w-24 h-24 sm:w-28 sm:h-28 md:w-36 md:h-36 rounded-full flex items-center justify-center text-4xl sm:text-5xl border-2"
                style={{
          ...
          <h1
            className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-bold mb-2 break-words"
            style={{
              fontFamily: theme.scriptFont,
              color: theme.primaryColor,
          ...
          <h1
            className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-bold mb-6 sm:mb-8 break-words"
            style={{
              fontFamily: theme.scriptFont,
              color: theme.primaryColor,
              textShadow: `0 0 40px ${theme.primaryColor}30`,
          ...
          {guestName && (
            <div className="mt-6 inline-block">
              <div
                className="px-4 sm:px-6 py-3 rounded-2xl backdrop-blur-md border"
                style={{
                  background: theme.cardBg,
                  borderColor: `${theme.primaryColor}30`,
                }}
      ...
      {weddingData.quote && (
        <section className="py-16 sm:py-20 px-4 sm:px-6 relative">
          <div className="max-w-2xl mx-auto text-center">
            <div
              className="p-6 sm:p-8 md:p-12 rounded-3xl backdrop-blur-md border relative overflow-hidden"
              style={{
                background: theme.cardBg,
            ...
            >
              <p
                className="text-base sm:text-lg md:text-xl italic leading-relaxed mb-4"
                style={{ color: theme.textColor }}
              >
                "{weddingData.quote}"
              </p>
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto">
          <h2
            className="text-center text-2xl sm:text-3xl md:text-5xl font-bold mb-4"
          ...
          {weddingData.greeting && (
            <p
              className="text-center text-sm mb-8 sm:mb-12 opacity-60 px-4"
              style={{ color: theme.textColor }}
            >
              {weddingData.greeting}
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
            <div
              className="p-6 sm:p-8 rounded-3xl backdrop-blur-md border text-center relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300"
              style={{
                background: theme.cardBg,
                borderColor: `${theme.primaryColor}20`,
              }}
            >
              {weddingData.groomPhoto ? (
                <div
                  className="w-28 h-28 sm:w-32 sm:h-32 md:w-36 md:h-36 mx-auto mb-4 sm:mb-6 rounded-full overflow-hidden border-2 shadow-lg"
                ...
                <div
                  className="w-28 h-28 sm:w-32 sm:h-32 md:w-36 md:h-36 mx-auto mb-4 sm:mb-6 rounded-full flex items-center justify-center text-5xl sm:text-6xl border-2"
                  style={{
                    background: `${theme.primaryColor}10`,
                    borderColor: `${theme.primaryColor}30`,
                  }}
              ...
              <h3
                className="text-xl sm:text-2xl md:text-3xl font-bold mb-3 break-words"
                style={{ fontFamily: theme.headingFont, color: theme.primaryColor }}
              >
                {weddingData.groomName}
              </h3>
              ...
              {weddingData.groomParentsAddress && (
                <p
                  className="text-xs opacity-40 mt-2 break-words"
                  style={{ color: theme.textColor }}
                >
                  {weddingData.groomParentsAddress}
                </p>
          ...
          {weddingData.couplePhoto && (
            <div
              className="mt-8 sm:mt-12 rounded-3xl overflow-hidden border shadow-2xl"
          ...
          <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="grid grid-cols-4 gap-2 sm:gap-3 md:gap-5">
              {[
                ...
                <div
                  key={item.label}
                  className="p-3 sm:p-4 md:p-6 rounded-2xl backdrop-blur-md border"
                  style={{
                    background: theme.cardBg,
                    borderColor: `${theme.primaryColor}20`,
                  }}
                >
                  <p
                    ...
                    style={{
                      ...
                      textShadow: `0 0 20px ${theme.primaryColor}30`,
                    }}
                  >
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto">
          <h2
            className="text-center text-2xl sm:text-3xl md:text-5xl font-bold mb-8 sm:mb-12"
            style={{ fontFamily: theme.headingFont, color: theme.primaryColor }}
          >
            Acara
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {weddingData.weddingVenue && (
              <div
                className="p-6 sm:p-8 rounded-3xl backdrop-blur-md border text-center"
                style={{
                  background: theme.cardBg,
                  borderColor: `${theme.primaryColor}20`,
                }}
              >
                ...
                {weddingData.mapLink && (
                  <a
                    href={weddingData.mapLink}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 mt-5 px-5 py-2.5 rounded-full text-sm font-medium"
                    style={{
                      background: `linear-gradient(135deg, ${theme.primaryColor}, ${theme.accentColor})`,
                      color: '#fff',
                    }}
              ...
              >
                ...
                <div className="space-y-3" style={{ color: theme.textColor }}>
                  <p className="flex items-center justify-center gap-2 text-sm">
                    ...
                    />
                    <span className="break-words">
                      {formatDate(weddingData.receptionDate)}
                    </span>
                  </p>
                  <p className="flex items-center justify-center gap-2 text-sm">
                    <Clock
      ...
      {weddingData.story && (
        <section className="py-16 sm:py-20 px-4 sm:px-6">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <div
              className="p-6 sm:p-8 rounded-3xl backdrop-blur-md border"
              style={{
                background: theme.cardBg,
                borderColor: `${theme.primaryColor}20`,
              }}
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {weddingData.galleryImages.map((img, index) => (
                <div
                  key={index}
                  className="rounded-2xl overflow-hidden border shadow-lg aspect-square"
                  style={{ borderColor: `${theme.primaryColor}20` }}
                >
                  <img
                    src={img}
        ...
        <section className="py-16 sm:py-20 px-4 sm:px-6">
          <div className="max-w-2xl mx-auto text-center">
            <div className="flex items-center justify-center gap-3 mb-4">
              <div
                className="w-12 h-px"
                style={{ background: `${theme.primaryColor}40` }}
              />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8 sm:mt-10">
              {weddingData.bankName && (
                <div
                  className="p-5 sm:p-6 rounded-2xl backdrop-blur-md border"
                  style={{
                    background: theme.cardBg,
                    borderColor: `${theme.primaryColor}20`,
                  }}
                >
                  ...
                  <p
                    className="text-xl sm:text-2xl font-mono font-bold mb-1 break-all"
                    style={{ color: theme.textColor }}
                  >
                    {weddingData.bankAccount}
                  </p>
                  <p className="text-sm opacity-60" style={{ color: theme.textColor }}>
                    a.n. {weddingData.bankHolder}
                  </p>
                  <button
                    ...
                    style={{
                      background:
                        copiedAccount === 'bank1' ? '#10b981' : `${theme.primaryColor}15`,
                      color: copiedAccount === 'bank1' ? '#ffffff' : theme.primaryColor,
                      border: `1px solid ${
                        copiedAccount === 'bank1' ? '#10b981' : theme.primaryColor
                      }30`,
                    }}
                  >
              ...
              {weddingData.bankName2 && (
                ...
                >
                  ...
                  <p
                    ...
                    style={{ color: theme.textColor }}
                  >
                    {weddingData.bankAccount2}
                  </p>
                  <p className="text-sm opacity-60" style={{ color: theme.textColor }}>
                    a.n. {weddingData.bankHolder2}
                  </p>
                  <button
                    ...
                    style={{
                      background:
                        copiedAccount === 'bank2' ? '#10b981' : `${theme.primaryColor}15`,
                      color: copiedAccount === 'bank2' ? '#ffffff' : theme.primaryColor,
                      border: `1px solid ${
                        copiedAccount === 'bank2' ? '#10b981' : theme.primaryColor
                      }30`,
                    }}
                  >
              ...
              <div className="space-y-5">
                ...
                <div className="flex gap-3 justify-center flex-wrap">
                  <button
                    ...
                    className="px-6 sm:px-8 py-3 rounded-xl font-medium text-sm min-h-[44px]"
                    style={{
                ...
                {rsvpStatus === 'accepted' ? (
                  <>
                    ...
                    <select
                      ...
                      className="mt-2 p-3 rounded-xl border w-full text-sm min-h-[44px]"
                      style={{
                  ...
                  <p className="text-red-400 font-medium">
                    Sayang sekali Anda tidak bisa hadir.
                  </p>
                )}
              </div>
            )}
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-6">
        <div className="max-w-2xl mx-auto">
          ...
          <div
            className="p-4 sm:p-6 rounded-3xl backdrop-blur-md border mb-6"
            style={{
              background: theme.cardBg,
              borderColor: `${theme.primaryColor}20`,
            }}
          >
            <div className="space-y-3">
              <input
                ...
                className="w-full p-3 rounded-xl border focus:outline-none focus:ring-2 text-sm min-h-[44px]"
              ...
              <button
                onClick={submitWish}
                className="w-full py-3 rounded-xl font-medium text-sm flex items-center justify-center gap-2 min-h-[44px]"
                style={{
                  background: `linear-gradient(135deg, ${theme.primaryColor}, ${theme.accentColor})`,
                  color: '#fff',
                }}
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto">
            ...
            {wishes.length === 0 && (
              <p
                className="text-center text-sm py-4 opacity-40"
                style={{ color: theme.textColor }}
              >
                Belum ada ucapan.
              </p>
      ...
      >
        <p
          className="text-xl sm:text-2xl md:text-3xl font-bold mt-6 break-words"
          style={{
            color: theme.primaryColor,
            fontFamily: theme.scriptFont,
            textShadow: `0 0 20px ${theme.primaryColor}20`,
          }}
        >
        ...
        <a
          href="#/admin"
          className="inline-block mt-4 text-xs opacity-20 hover:opacity-50 transition"
          style={{ color: theme.textColor }}
        >
          Admin Panel
        </a>
